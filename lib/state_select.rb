...
module ActionView::Helpers::FormOptionsHelper
  ...
  # using state_options_for_select to generate the list of option tags.
  ...
  def state_select(object, method, country = "US", options = {}, html_options = {})
    ...
    ).to_state_select_tag(country, options, html_options)
  end

  # Returns a string of option tags for states in a country. Supply a state
  # name as +selected+ to have it marked as the selected option tag.
  #
  # NOTE: Only the option tags are returned, you have to wrap this call in a regular HTML select tag.

  def state_options_for_select(selected = nil, country = "US", options = {})
    state_options = ""

    if country
      countries = eval(country.upcase + "_STATES").dup

      countries.unshift("") if options[:add_empty]
      state_options += options_for_select(countries, selected)
    end
...
class ActionView::Helpers::InstanceTag
  def to_state_select_tag(country, options, html_options)
    ...
    content_tag("select", add_options(state_options_for_select(selected_value, country, options), options, value), html_options)
  end
end
